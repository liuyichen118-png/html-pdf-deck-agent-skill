...
| 参数 | 用途 |
|---|---|
| `--viewport=1280x720` | 设置渲染画布尺寸。 |
| `--output-name=name` | 覆盖默认输出文件名。 |
| `--slide-selector=.slide` | 指定固定页面元素的 CSS 选择器。 |
| `--vector-only` | 只导出 Chromium vector PDF 和 `qa.json`，用于调试。 |
...
作者名保留为 `61X-Studio`，许可证为 MIT。

后续发布新版本前，需要重新运行：

- smoke / CJK smoke 测试。
- dependency audit。
- secret/private-path 扫描。
- GitHub README 和 demo 图片检查。
